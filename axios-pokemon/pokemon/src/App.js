    ...
    <div>
      <div></div>
        <div>
          <div>
            <div>
              <label forHtml="searchpoke">Search here or click a Pokemon below:</label>
              <input type="text" name="searchpoke" className="form-control " onChange={e => setsearch(e.target.value)}/>
              <button type="text" name="searchpoke" className="form-control " onClick={e => setsearch(e.target.value)}/>
            </div>

            <ul>
              { display.map((poke, i) => 
                <li key={i} onClick={e => getdetails(poke)}>{poke.name}</li>
